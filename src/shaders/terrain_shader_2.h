...
namespace octet {
  ...
  class terrain_shader : public shader {
	...
	// index for tiling factor;
	GLuint tilingFactor_index;
  ...
  public:
      ...
      const char fragment_shader[] = SHADER_STR(
        ...
        void main() {
			...
			*/
			...
			float nDotL = max(0.0f, dot(nNorm, light_direction);
    ...
    void init_uniforms(const char *vertex_shader, const char *fragment_shader) {
	  ...
	  tilingFactor_index = glGetUniformLocation(program(), "tilingFactor"); 
    }

    void render(const mat4t &modelToProjection, const mat4t &modelToCamera, const vec4 &light_direction, float shininess, vec4 &light_ambient, vec4 &light_diffuse, vec4 &light_specular, float tiling_factor, int num_samplers = 6) {
	  ...
	  glUniform1f(tilingFactor_index, tiling_factor); 

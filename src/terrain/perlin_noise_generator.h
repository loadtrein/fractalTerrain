...
namespace octet {
  class PerlinNoiseGenerator {
    ...
    float _gradients[GradientSizeTable * 3];

    byte _perm[256];

    void InitGradients() {
      for (int i = 0; i < GradientSizeTable; i++){
        float z = 1.0f - 2.0f * static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/0.99));
        float r = (float)sqrt(1.0f - z * z);
        float theta = 2 * (float)3.14159 * static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/0.99));
        _gradients[i * 3] = r * (float)cos(theta);
